...
namespace MupdfSharp
{
	static class PageRenderer
	{
		public static Bitmap[] Render(string path, int height, params int[] pageIndexes) {
            ...
            {
                pageIndexes = new int[pn];
                for (int i = 0; i < pn; i++)
                    pageIndexes[i] = i;
            }
            Bitmap[] bmps = new Bitmap[pageIndexes.Length];
            foreach (int i in pageIndexes) { // iterate through each pages
                try
                {
		...
		static Bitmap RenderPage (IntPtr context, IntPtr document, IntPtr page, Rectangle pageBound, float zoom) {
			Matrix ctm = new Matrix ();
			IntPtr pix = IntPtr.Zero;
			IntPtr dev = IntPtr.Zero;

			int width = (int)(zoom * (pageBound.Right - pageBound.Left)); // gets the size of the page
			int height = (int)(zoom * (pageBound.Bottom - pageBound.Top));
			ctm.A = ctm.D = zoom; // sets the matrix as the identity matrix (1,0,0,1,0,0)

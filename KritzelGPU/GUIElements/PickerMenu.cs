...
namespace Kritzel.Main
{
    ...
    public partial class PickerMenu : UserControl
    {
        ...
        public static int SIZE { get { return Util.GetGUISize(); } }
        ...
        void draw(Graphics g)
        {
            if (contents != null)
            {
                if (Orientation == PickerOrientation.Horizontal)
                {
                    ...
                    {
                        Rectangle rect = new Rectangle(i * SIZE, 0, SIZE, this.Height);
                        Brush b = new SolidBrush(Style.Default.MenuBackground);
                        if (rect.Contains(cursor)) b = new SolidBrush(Style.Default.MenuContrast);
                        if (isSelected(i)) b = new SolidBrush(Style.Default.Selection);

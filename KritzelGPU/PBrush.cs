...
namespace Kritzel.Main
{
    public class PBrush
    {
        ...
        {
            ColorFilter filter = ColorFilter.Normal;
            if (MainWindow.Instance != null && MainWindow.Instance.GetDocument() != null)
                filter = MainWindow.Instance.inkControl1.Page.Filter;
            return Util.ApplyFilter(colors[0], filter);
        }
